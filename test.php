...
Pomm::createConnection("local", array('dsn' => 'pgsql://greg/greg'));
Pomm::createConnection("host", array('dsn' => 'pgsql://greg@localhost/greg'));

$result = Pomm::executeAnonymousQuery('SELECT 4', 'local');
print_r($result->fetch());

try
{
    $result = Pomm::executeAnonymousQuery('SELECT 4', 'host');
}
catch(\Pomm\Exception\Exception $e)
{
    printf("Pomm Exception caught with message '%s'\n", $e->getMessage());
}
Pomm::createConnection("host", array('dsn' => 'pgsql://test:test0!@localhost/greg'));
$result = Pomm::executeAnonymousQuery('SELECT 4<>1', 'host');
print_r($result->fetch());
