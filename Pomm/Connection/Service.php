...
class Service
{
    ...
    {
        return new Observer($this->connection);
    }
     ...
     * @return Service
     */
    protected function notify($name, $payload = null)
    {
        ...
        try
        {
            if (empty($payload))
            {
                $this->connection->executeAnonymousQuery(sprintf("NOTIFY %s", $name));
            }
            else
            {
                $this->connection->executeAnonymousQuery(sprintf("NOTIFY %s, %s", $name,  $this->connection->escapeLiteral($payload)));
            }
        }
        catch(ConnectionException $e)
        {
            $this->connection->log(LogLevel::ERROR, sprintf("Could not notify '%s' event.", $name));
